...
func TestAccountGetChatIdentityTokenWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Accounts.GetChatIdentityToken(context.TODO(), gitpod.AccountGetChatIdentityTokenParams{
		Empty: gitpod.F(true),
	})
	if err != nil {
		var apierr *gitpod.Error
